...
public class TicketsServiceImpl extends ServiceImpl<TicketsMapper, Tickets> implements TicketsService {
    ...
    @Override
    @CacheEvict(allEntries = true)
    public boolean update(Tickets entity, QueryWrapper query) {
        return super.update(entity, query);
    }

    @Override
    @CacheEvict(key = "#entity.ticketId")
    public boolean updateById(Tickets entity, boolean ignoreNulls) {
        return super.updateById(entity, ignoreNulls);
    }

    @Override
    @CacheEvict(allEntries = true)
    public boolean updateBatch(Collection<Tickets> entities, int batchSize) {
        return super.updateBatch(entities, batchSize);
    }

    @Override
    @Cacheable(key = "#id")
    public Tickets getById(Serializable id) {
        return super.getById(id);
    }

    @Override
    @Cacheable(key = "#root.methodName + ':' + #query.toSQL()")
    public Tickets getOne(QueryWrapper query) {
    ...
    @Override
    @Cacheable(key = "#root.methodName + ':' + #query.toSQL()")
    public List<Tickets> list(QueryWrapper query) {
    ...
    @Override
    @Deprecated
    public List<Tickets> listByIds(Collection<? extends Serializable> ids) {
